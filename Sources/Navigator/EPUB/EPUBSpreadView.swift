...
class EPUBSpreadView: NSView, Loggable, PageView {
    ...
    weak var activityIndicatorView: NSProgressIndicator?
...
//        NotificationCenter.default.addObserver(self, selector: #selector(voiceOverStatusDidChange), name: UIAccessibility.voiceOverStatusDidChangeNotification, object: nil)
    ...
    var scrollView: NSScrollView {
        webView.enclosingScrollView ?? NSScrollView()
    }
    ...
    @objc private func voiceOverStatusDidChange() {
        guard isVoiceOverRunning != NSWorkspace.shared.isVoiceOverEnabled else {
            return
        }
...
extension EPUBSpreadView {
    @objc func scrollViewDidScroll(_ notification: Notification) {
        // This replaces func scrollViewDidScroll(_ scrollView: UIScrollView)
        // Overridden in subclasses to track progress.
    }
}
...
extension EPUBSpreadView: NSGestureRecognizerDelegate {
    func gestureRecognizer(_ gestureRecognizer: NSGestureRecognizer, shouldRequireFailureOf otherGestureRecognizer: NSGestureRecognizer) -> Bool {
        // The logic remains the same, just using the AppKit protocol name.
        true
    }
}
...
private extension EPUBSpreadView {
    ...
    func createActivityIndicator(color: NSColor) {
        guard activityIndicatorView?.isIndeterminate == true else {
            return
        }
        ...
        activityIndicatorView = progressIndicator
    }
    ...
    private func setNeedsStopActivityIndicator() {
        ...
        activityIndicatorStopWorkItem = DispatchWorkItem { [weak self] in
            ...
            guard
                let self = self,
                let workItem = self.activityIndicatorStopWorkItem,
                !workItem.isCancelled
            else {
                return
            }

            trace("stopping activity indicator because spread \(self.spread.first.link.href) did not load")
